...
import { SustainabilityProofDocument as SustainabilityProofDocumentTemplate } from '../generated/templates'
...
export function handleNewDeposit(event: NewDepositEvent): void {
  saveRewardTransfer(eventId(event), event.params.appId, '', event.params.amount, 'DEPOSIT', event.params.depositor, null, null, null, event.address, event.block.number, event.block.timestamp, event.transaction.hash, event.logIndex)
}

export function handleTeamWithdrawal(event: TeamWithdrawalEvent): void {
  saveRewardTransfer(eventId(event), event.params.appId, '', event.params.amount, 'WITHDRAW', event.params.withdrawer, event.params.teamWallet, event.params.reason, null, event.address, event.block.number, event.block.timestamp, event.transaction.hash, event.logIndex)
}

export function handleRewardDistribution(event: RewardDistributedEvent): void {
  const id = eventId(event)
  saveRewardTransfer(id, event.params.appId, '', event.params.amount, 'DISTRIBUTION', event.params.distributor, event.params.receiver, null, event.params.proof, event.address, event.block.number, event.block.timestamp, event.transaction.hash, event.logIndex)
  saveSustainabilityProofDocument(id, event.params.proof, event.params.appId, event.params.receiver, event.params.amount, event.address, event.block.number, event.block.timestamp, event.transaction.hash, event.logIndex)
}
...
export function handleAllocationRewardsClaimed(event: AllocationRewardsClaimedEvent): void {
  ...
  entity.round = event.params.roundId.toString()
  entity.targetRound = event.params.roundId.plus(BigInt.fromI32(1)).toString()
...
function saveSustainabilityProofDocument(
  ...
  if (!proof.startsWith('{')) return
  saveInlineSustainabilityProof(transferId.toHexString().concat('/proof'), proof, app, transferId, '', account, rewardExact, sourceBlock, sourceTimestamp, sourceTxHash, sourceLogIndex, emitter)
}
