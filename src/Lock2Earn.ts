...
function incrementLock2EarnTermRewards(termId: string, reward: BigInt): void {
    let term = Lock2EarnTerm.load(termId)
    if (term == null) { return }

    let stats = fetchLock2EarnStats()
    term.rewardsExact = term.rewardsExact.plus(reward)
    term.rewards = decimals.toDecimals(term.rewardsExact, 18)
    term.save()
    stats.totalRewardsExact = stats.totalRewardsExact.plus(reward)
    stats.totalRewards = decimals.toDecimals(stats.totalRewardsExact, 18)
    stats.save()
}

// Find Lock2EarnTerm by veDelegatePool address (Account id)
function findLock2EarnTermByPoolAddress(poolAddress: Address): VeDelegateAccount | null {
    let veDelegateAccount = VeDelegateAccount.load(poolAddress)
    if (veDelegateAccount && veDelegateAccount.asLock2EarnTerm) {
        return veDelegateAccount
    }
    return null
}
...
export function handleTermAdded(event: TermAddedEvent): void {
    ...
    if (!poolAddressResult.reverted) {
        let veDelegatePoolAccount = fetchVeDelegateAccount(poolAddressResult.value)
        veDelegatePoolAccount.lock2EarnTermId = term.id
        veDelegatePoolAccount.save()

        term.veDelegateAccount = veDelegatePoolAccount.id
    }
...
export function handleTermClosed(event: TermClosedEvent): void {
    ...
    stats.totalRewardsExact = stats.totalRewardsExact.minus(term.rewardsExact)
    stats.totalRewards = decimals.toDecimals(stats.totalRewardsExact, 18)
    stats.save()
...
export { incrementLock2EarnTermRewards, findLock2EarnTermByPoolAddress }
